...
class tsAStockFinance:
        ...
        for ts_code in stock_list:
            ...
            while True:
                try:
                    ...
                    if report_type > 0:
                        if len(end_list) > 1:
                            df = f(ts_code=ts_code, start_date=end_list[0],
                                   end_date=datetime.datetime.now().strftime('%Y%m%d'), fileds=fileds,
    ...
    def disclosure_date(pro, db):
        end_date_list = ['0331', '0630', '0930', '1231']
        engine = mysql.getDBEngine(db)
        lastdate = tsSHelper.getLastDateAndDelete(table='astock_finance_disclosure_date', filed='end_date', ts_code="",
                                                  db=db)
            ...
            for i in range(0, 100):
                while True:
                    try:
                        ...
                        df.to_sql('astock_finance_disclosure_date', engine, index=False, if_exists='append',
                                  chunksize=5000)
                        break
                    except Exception as e:
                        if "最多访问" in str(e):
